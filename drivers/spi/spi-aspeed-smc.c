					  ...
					  const struct spi_mem_op *op)
{
	...
	dev_info(dev, "cs:%d, cmd:%x(%d),addr:%llx(%d),dummy:%d(%d),data_len:%x(%d)\n",
	...
	dev_info(dev, "ctrl: 0x%08x, addr_mode: 0x%x, mask: 0x%x, addr:0x%p\n",
					       ...
					       u64 offs, size_t len, void *buf)
{
	...
	dev_info(dev, "read op:0x%x, addr:0x%llx, len:0x%zx\n",
						...
						const void *buf)
{
	...
	dev_info(dev, "write op:0x%x, addr:0x%llx, len:0x%zx\n",
		op_tmpl.cmd.opcode, offs, len);
						...
						u64 offs, size_t len, const void *buf)
{
	...
	dev_info(dev, "write op:0x%x, addr:0x%08llx, len:0x%08zx\n",
		op_tmpl.cmd.opcode, offs, len);
...
static int aspeed_spi_set_window(struct aspeed_spi *aspi)
{
	...
	for (cs = 0; cs < aspi->data->max_cs; cs++) {
		...
		if (win_sz != 0)
			dev_info(dev, "CE%d new window [ 0x%.9llx - 0x%.9llx ] %zdMB",
				cs, (u64)start, (u64)end - 1,  win_sz >> 20);
		else
			dev_info(dev, "CE%d window closed", cs);
...
static int aspeed_spi_dirmap_create(struct spi_mem_dirmap_desc *desc)
{
	...
	if (op->data.dir == SPI_MEM_DATA_IN) {
		...
		dev_info(aspi->dev, "spi clock frequency: %dMHz\n",
...
static int aspeed_spi_setup(struct spi_device *spi)
{
	...
	dev_info(aspi->dev, "CE%d setup done\n", cs);
	return 0;
}
...
static void aspeed_spi_cleanup(struct spi_device *spi)
{
	...
	dev_info(aspi->dev, "CE%d cleanup done\n", cs);
}
				      ...
				      u32 max_hz)
{
	...
	dev_info(dev, "found: %s, hclk: %d, max_clk: %d\n",
		found ? "yes" : "no", hclk_clk, max_hz);

	if (found) {
		dev_info(dev, "h_div: %d (mask 0x%08x), speed: %d\n",
				      ...
				      u32 max_hz)
{
	...
	if (found) {
		dev_info(dev, "h_div: %d (mask %x), speed: %d\n",
			i + 1, hclk_div, chip->clk_freq);
	}
				      ...
				      u32 max_hz)
{
	...
	if (found) {
		dev_info(dev, "base_clk: %d, h_div: %d (mask %x), speed: %d\n",
				...
				const u8 *golden_buf, u8 *test_buf)
{
	...
	dev_info(aspi->dev, " * -> good is pass %d [0x%08x]",
...
static int aspeed_spi_do_calibration(struct aspeed_spi_chip *chip)
{
	...
	dev_info(aspi->dev, "calculate timing compensation - AHB freq: %d MHz",
		ahb_freq / 1000000);
	...
	for (i = data->hdiv_max; i <= 5; i++) {
		...
		writel(tv, chip->ctl);
		dev_info(aspi->dev, "Trying HCLK/%d [%08x] ...", i, tv);
...
no_calib:
	...
	if (best_div < 0) {
		...
		dev_info(aspi->dev, "Found good read timings at HCLK/%d", best_div);
		chip->clk_freq = freq;
					...
					const u8 *golden_buf, u8 *test_buf)
{
	...
	dev_info(dev, "final hcycle: %d, delay_ns: %d\n", hcycle,
		delay_ns);
